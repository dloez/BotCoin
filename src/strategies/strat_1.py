...
from listener import STATUS_LISTENING
...
class Strat1(Strategy):
    ...
    def run(self):
        ...
        while True:
            ...
            if buy_signal or sell_signal:
                last_order = self.__session.query(dbmanager.Order).order_by(dbmanager.Order.id.desc()).first()

            if buy_signal and self._listener.status != STATUS_LISTENING:
                if not last_order or last_order.side == SIDE_SELL:
                    self._buy()
    ...
    def _buy(self):
        ...
        stop_loss = adjust_size(price * ((100 - self.data.loss) / 100), tick_size=self._symbol_assets['tick_size'])
        self._listener.attach(take_profit_order, stop_loss)
